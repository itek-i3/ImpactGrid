...
import { Wallet, Plus, Trash2, TrendingUp, TrendingDown, Sigma, ChevronDown, ChevronRight, Lock, X } from 'lucide-react';
...
export default function FinancePanel() {
  ...
  const demoKey = agencyId ? `demo-finance-${agencyId}` : 'demo-finance';
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!agencyId) { if (!cancelled) { setRows([]); setLoading(false); } return; }
      const { data } = await createClient()
        .from('daily_finance').select('*').eq('agency_id', agencyId)
  ...
  // Group entries into Monday-based weeks, newest week first.
  const weeks = useMemo(() => {
    const map = new Map();
    rows.forEach(r => {
      const start = mondayOf(r.entry_date);
      const key = start.toISOString().slice(0, 10);
      if (!map.has(key)) map.set(key, { key, start, rows: [] });
      map.get(key).rows.push(r);
    });
    const arr = [...map.values()];
    arr.forEach(w => {
      w.rows.sort((a, b) => (a.entry_date < b.entry_date ? 1 : a.entry_date > b.entry_date ? -1 : 0));
      let rev = 0, exp = 0;
      w.rows.forEach(r => { rev += num(r.revenue); exp += num(r.expenses); });
      w.totals = { revenue: rev, expenses: exp, net: rev - exp };
    });
    arr.sort((a, b) => b.start - a.start);
    return arr;
  }, [rows]);

  const latestWeekKey = weeks[0]?.key;
  const isWeekOpen = (key) => (key === latestWeekKey) !== weekToggles.has(key); // latest open by default
    ...
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '8px 4px 40px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12.5, color: 'var(--color-text-tertiary)' }}>Log daily revenue and itemized expenses — grouped by week, shared with your agency</div>
        </div>
      </div>
        ...
        <div style={{ ...card, border: '1px solid rgba(34,197,94,0.35)', marginBottom: 22 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 14, flexWrap: 'wrap', marginBottom: 14 }}>
            ...
            <div style={{ marginLeft: 'auto', textAlign: 'right' }}>
              <div style={{ fontSize: 10.5, color: 'var(--color-text-tertiary)', textTransform: 'uppercase', letterSpacing: '.05em' }}>Net</div>
              <div style={{ fontSize: 17, fontWeight: 800, color: netColor(num(nRevenue) - itemsTotal(nItems)), fontVariantNumeric: 'tabular-nums' }}>{money(num(nRevenue) - itemsTotal(nItems))}</div>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {weeks.map(week => {
            ...
            return (
              <div key={week.key}>
                {/* Week header */}
                <button className="fin-week" onClick={() => toggleWeek(week.key)}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
                    {open ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
                    <span style={{ fontSize: 13.5, fontWeight: 800, color: 'var(--color-text-primary)' }}>Week of {weekRange(week.start)}</span>
                    <span style={{ fontSize: 11, color: 'var(--color-text-tertiary)' }}>· {week.rows.length} day{week.rows.length !== 1 ? 's' : ''}</span>
                  </span>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 14, fontVariantNumeric: 'tabular-nums', flexShrink: 0 }}>
                    <span style={{ fontSize: 12, color: '#22C55E', fontWeight: 700 }}>{money(week.totals.revenue)}</span>
                    <span style={{ fontSize: 12, color: '#E0485A', fontWeight: 700 }}>{money(week.totals.expenses)}</span>
                    <span style={{ fontSize: 13, color: netColor(week.totals.net), fontWeight: 800 }}>Net {money(week.totals.net)}</span>
                  </span>
                </button>
                ...
                {open && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 8 }}>
                    {week.rows.map(row => {
                      const net = num(row.revenue) - num(row.expenses);
                      ...
                      return (
                        <div key={row.id} style={{ background: 'var(--color-bg-secondary)', border: `1px solid ${dayOpen ? 'var(--color-border-active)' : 'var(--color-border)'}`, borderRadius: 12, overflow: 'hidden' }}>
                          {/* Compact row — on phones show only date + net so it fits */}
                          <div onClick={() => setOpenDayId(dayOpen ? null : row.id)}
                            style={{ display: 'grid', gridTemplateColumns: isMobile ? '16px 1fr auto 28px' : '18px 1fr auto auto auto 30px', gap: 10, alignItems: 'center', padding: '10px 14px', cursor: 'pointer' }}>
                            ...
                            {!isMobile && <span style={{ fontSize: 12.5, color: '#22C55E', fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>{money(row.revenue)}</span>}
                            {!isMobile && <span style={{ fontSize: 12.5, color: '#E0485A', fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>−{money(row.expenses)}</span>}
                            <span style={{ fontSize: 13, color: netColor(net), fontWeight: 800, fontVariantNumeric: 'tabular-nums' }}>{money(net)}</span>
