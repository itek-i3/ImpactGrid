  ...
  addPage: async (page) => {
    ...
    if (isDemoMode()) {
      const newPage = {
        ...
        isPersonal: page.isPersonal || false,
        createdBy: state.userProfile?.id || null,
    ...
    try {
      const res = await fetch('/os/api/pages', {
        ...
        body: JSON.stringify({
          ...
          isPersonal: optimisticPage.isPersonal,
        }),
      });
        ...
        pages: state.pages.map((p) =>
          p.id === tempId
            ? {
                ...
                isPersonal: createdPage.is_personal,
                createdBy: createdPage.created_by,
